...
<body>
  ...
  <div class="container2">
        ...
        <div class="col-sm-5">
          <a href="details.php?news_id=<?= urlencode($mainNews['id']) ?>" class="text-decoration-none; color: inherit;">
            <div class="card bg-dark" style="height: 100%">
              <img src="<?= htmlspecialchars($mainNews['image_url']) ?>" alt="image" class="card-img-top" />
              <div class="card-body">
    ...
    <div class="secondSection row section mt-4">
      ...
      <div class="col-sm-6 mostReaded mt-3">
        ...
        <hr />
        <ul class="mostReadedList ps-0" id="mostCommentedList">
          <?php foreach ($mostCommented as $i => $item): ?>
            ...
            <?php if ($i < count($mostCommented) - 1): ?>
              <hr /><?php endif; ?>
          <?php endforeach; ?>
        </ul>
      </div>
  ...
  <script>
    function fetchMostCommented() {
      $.ajax({
        url: '../FetchMostCommented.php',
        type: 'GET',
        success: function (data) {
          $('#mostCommentedList').html(data);
        },
    ...
    fetchMostCommented();
    setInterval(fetchMostCommented, 3000);
  </script>
